...
static void _fill_ctld_conf(slurm_conf_t *conf_ptr)
{
	...
	conf_ptr->health_check_timeout = conf->health_check_timeout;
	...
	conf_ptr->metrics_auth_users = xstrdup(conf->metrics_auth_users);
	conf_ptr->metrics_params = xstrdup(conf->metrics_params);
...
static void _slurm_rpc_update_node(slurm_msg_t *msg)
{
	...
	if (error_code) {
		...
		debug2("%s complete for %s %s",
		       __func__, update_node_msg_ptr->node_names, TIMER_STR());
		slurm_send_rc_msg(msg, SLURM_SUCCESS);
	}
